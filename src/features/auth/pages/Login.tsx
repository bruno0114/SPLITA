...
import { Mail, Lock, ArrowRight, Facebook, Loader2 } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin, onRegister }) => {
   ...
   const { signInWithPassword, signInWithOAuth, user, loading: authLoading } = useAuth();
         ...
         <div className="w-full max-w-md bg-surface/80 backdrop-blur-xl border border-white/20 dark:border-white/5 shadow-2xl rounded-3xl p-8 md:p-10 relative z-10 animate-in fade-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
               ...
               <button
                  type="submit"
                  disabled={loading}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-gradient h-14 text-white font-bold text-lg shadow-[0_10px_30px_rgba(0,122,255,0.3)] hover:brightness-110 hover:scale-[1.01] transition-all active:scale-95 disabled:opacity-70 disabled:pointer-events-none"
               >
                  {loading ? (
            ...
            <div className="grid grid-cols-2 gap-3">
               <button
                  ...
                  className="flex items-center justify-center gap-2 h-12 rounded-xl bg-white text-slate-700 font-bold border border-slate-200 hover:bg-slate-50 transition-colors shadow-sm text-sm"
               >
               ...
               <button
                  ...
                  className="flex items-center justify-center gap-2 h-12 rounded-xl bg-[#1877F2] text-white font-bold hover:bg-[#166fe5] transition-colors shadow-sm text-sm"
               >
