    ...
    <aside className={`relative border-r border-border bg-surface/80 backdrop-blur-md flex flex-col h-full transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isCollapsed ? 'w-[80px]' : 'w-[260px]'}`}>
      ...
      <div className="p-4 border-t border-border bg-slate-50/50 dark:bg-black/20 backdrop-blur-sm shrink-0">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
          <div className={`size-9 rounded-full border-2 border-white dark:border-slate-700 overflow-hidden bg-cover bg-center shrink-0 shadow-sm transition-all duration-300 ${isCollapsed ? 'scale-100' : 'scale-100'}`} style={{ backgroundImage: `url(${CURRENT_USER.avatar})` }}>
          </div>
          <div className={`flex-1 min-w-0 transition-all duration-300 overflow-hidden ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
            <p className="text-xs font-bold truncate text-slate-900 dark:text-white">Juan Pérez</p>
            <p className="text-[10px] text-slate-500 font-semibold tracking-wide truncate">Plan Familiar</p>
          </div>
          {!isCollapsed && (
            <button 
              onClick={onLogout}
...
const NavItem = ({ active, icon, label, onClick, isCollapsed, highlight }: { active: boolean; icon: React.ReactNode; label: string; onClick: () => void, isCollapsed: boolean, highlight?: boolean }) => (
  <li>
    <button 
      ...
      className={`w-full flex items-center h-11 rounded-xl transition-all duration-200 group relative overflow-hidden mb-1 ${
        active 
