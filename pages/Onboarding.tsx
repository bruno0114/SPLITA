...
import { ChevronRight, X, Heart, User, Users, Check, ArrowRight, Link as LinkIcon, Copy, Facebook, Mail, Lock, Smartphone, ArrowLeft } from 'lucide-react';
import { GROUP_MEMBERS, CURRENT_USER } from '../constants';
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-background text-slate-900 dark:text-white transition-colors">
       ...
       <main className="flex flex-1 flex-col items-center justify-center py-8 px-6 z-10">
          ...
          {step === 2 && <StepUsageType onNext={handleNext} onBack={handleBack} progress={getProgress()} />}
          {step === 3 && <StepCreateGroup onNext={handleNext} onBack={handleBack} progress={getProgress()} />}
          {step === 4 && <StepAddPeople onNext={handleNext} onBack={handleBack} progress={getProgress()} />}
          {step === 5 && <StepSettings onNext={handleNext} onBack={handleBack} progress={getProgress()} />}
          {step === 6 && <StepCreateAccount onNext={handleNext} onBack={handleBack} progress={getProgress()} onLogin={onLogin} />}
       </main>
    </div>
  );
};
...
const StepUsageType = ({ onNext, onBack, progress }: { onNext: () => void, onBack: () => void, progress: number }) => {
  const [selected, setSelected] = useState<string>('couple');
...
const StepCreateGroup = ({ onNext, onBack, progress }: { onNext: () => void, onBack: () => void, progress: number }) => (
...
const StepAddPeople = ({ onNext, onBack, progress }: { onNext: () => void, onBack: () => void, progress: number }) => {
  ...
  return (
    <div className="flex flex-col w-full max-w-[480px] gap-8 animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      <div className="flex flex-col gap-4">
         ...
         <div className="flex gap-2 flex-wrap min-h-[44px]">
            <div className="flex h-11 shrink-0 items-center justify-center gap-x-2 rounded-full bg-blue-600/20 border border-blue-500/30 pl-3 pr-4 shadow-sm">
               ...
               <p className="text-slate-900 dark:text-white text-sm font-semibold">Matias (Vos)</p>
            </div>
            {GROUP_MEMBERS.slice(0, 2).map(user => (
               <div key={user.id} className="flex h-11 shrink-0 items-center justify-center gap-x-2 rounded-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 pl-3 pr-2 shadow-sm">
                  <User className="w-5 h-5 text-slate-400" />
                  <p className="text-slate-700 dark:text-white text-sm font-medium">{user.name}</p>
                  <button className="ml-1 text-slate-400 hover:text-red-400 transition-colors">
                     <X className="w-4 h-4" />
                  </button>
               </div>
            ))}
      ...
      <button className="flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-500 h-16 text-slate-500 dark:text-slate-400 font-bold text-lg hover:bg-blue-50 dark:hover:bg-blue-900/10 hover:text-blue-600 dark:hover:text-blue-400 transition-all active:scale-95">
      ...
      <ButtonGroup onNext={onNext} onBack={onBack} nextLabel="Siguiente paso" />
    </div>
  );
};

const StepSettings = ({ onNext, onBack, progress }: { onNext: () => void, onBack: () => void, progress: number }) => (
  <div className="flex flex-col w-full max-w-[480px] gap-8 md:gap-10 animate-in fade-in slide-in-from-right-8 duration-500">
    ...
    <div className="flex flex-col gap-3">
       <SettingToggle title="Detectar gastos repetitivos" desc="Sugerencias automáticas para abonos mensuales" checked={true} />
       <SettingToggle title="Dividir 50/50 por defecto" desc="Ahorrá tiempo en la carga de gastos" checked={true} />
       <SettingToggle title="Notificar nuevos gastos" desc="Avisar a todos cuando alguien suma un ticket" checked={false} />
    </div>
...
const StepCreateAccount = ({ onNext, onBack, progress, onLogin }: { onNext: () => void, onBack: () => void, progress: number, onLogin: () => void }) => (
   <div className="flex flex-col w-full max-w-[480px] gap-8 animate-in fade-in slide-in-from-right-8 duration-500">
     <div>
       <ProgressBar percent={progress} />
     ...
     <div className="space-y-4">
       <div className="grid grid-cols-2 gap-3">
          <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-white text-slate-700 font-bold border border-slate-200 hover:bg-slate-50 transition-colors shadow-sm">
          ...
          <button className="flex items-center justify-center gap-2 h-14 rounded-2xl bg-[#1877F2] text-white font-bold hover:bg-[#166fe5] transition-colors shadow-sm">
       ...
       <div className="space-y-3">
          <div className="relative">
             ...
             <input type="email" placeholder="Correo electrónico" className="w-full h-14 bg-surface border border-slate-200 dark:border-slate-800 rounded-2xl pl-12 pr-4 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
          </div>
          <div className="relative">
             <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
             <input type="password" placeholder="Contraseña" className="w-full h-14 bg-surface border border-slate-200 dark:border-slate-800 rounded-2xl pl-12 pr-4 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
          </div>
       </div>
 
       <ButtonGroup onNext={onNext} onBack={onBack} nextLabel="Crear cuenta" />
...
const SettingToggle = ({ title, desc, checked }: { title: string, desc: string, checked: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${isOn ? 'bg-blue-600/30 border border-blue-500/50' : 'bg-slate-300 dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
         <div className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-300 ${isOn ? 'translate-x-6 bg-blue-gradient' : 'translate-x-0 bg-slate-500'}`} />
      </div>
    </div>
  );
};
